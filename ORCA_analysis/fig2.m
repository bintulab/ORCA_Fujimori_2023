...
for i = 1:conditionnum
    ...
    tmp = tmp(~isnan(tmp));
    bar(i,nanmean(tmp),0.65,'FaceColor',cmap(idxkm,:),'LineWidth',2);
...
idx2 = 3;%23; % krab 5 days
 idx1 = 12;%16;%7;%21;% % irr
 idx2 = 15;%20;%11;%22;% % irr
 idx1 = 16;%26;%19;%
 idx2 = 19;%28;%22;%
binnum = 30;
binr = [50,400];
...
for idxobj = 1:length(Objs)
    for jj = 1:length(Objs(idxobj).df)
        if sum(strcmp(Objs(idxobj).df(jj).dname,dataselected))
            dfallcoordfilt = cat(2,dfallcoordfilt,Objs(idxobj).df(jj));
        end
    end
end
rehyb = [];
...
%% re-run tSNE
allpairs = [];
for ii = 1:size(dmatinterp,3)
    tmpmat = dmatinterp(4:16,4:16,ii);
    tmp_triu = ~triu(ones(size(tmpmat,1),size(tmpmat,2)))';
    allpairs = cat(1,allpairs,tmpmat(tmp_triu)');
end
rng('default')
tSNEoutN = tsne(allpairs,'Perplexity',35,'Algorithm','barneshut','Distance','euclidean');
...
for ii = 1:2
    ...
    dscatter(tSNEoutN(idx,1),tSNEoutN(idx,2));
    xlabel('t-SNE 1'); ylabel('t-SNE 2');
    title(dname);
    axis image; box on;
    axis([-50,50,-50,50]);
...
for ii = 1:2
    ...
    tmp1 = dfall(visidx(ii)).tSNEout(:,2);
    h1 = histogram(tmp1,binnum,'BinLimits',binr,'normalization','probability',...
        'EdgeAlpha',0.0,'FaceColor',cmap(ii,:));
    stairs([h1.BinEdges,h1.BinEdges(end)],[h1.Values,h1.Values(end),0],'k-');
...
hold on;
    switch ii 
        case 1
            tmp1 = Rg1(tmptsne(:,1) < 0);
            tmp1 = tmp1(1:550);
            tmp2 = Rg1(tmptsne(:,1) > 0);
            tmp2 = tmp2(1:677);
        case 2
            tmp1 = Rg2(tmptsne(:,1) < 0);
            tmp1 = tmp1(1:550);
            tmp2 = Rg2(tmptsne(:,1) > 0);
            tmp2 = tmp2(1:677);
    end
