   ...
   df1 = Objs(kk).df;
    ...
    for ii = 1:length(drange)
        if sum(strcmp(df1(ii).dname,dataselected))
            ...
            dfig5.coordIntp = cat(3,dfig5.coordIntp,df1(ii).coordinterp(1:19,:,:));
            for jj = 1:dnum
...
%         end
        ...
        tmp = dfig5.coordIntp(:,:,strcmp(dfig5.dname,dataselected(ii)) & dfig5.rep == jj);
        tmp = reshape(nanmedian(sqrt(sum((tmp-repmat(mean(tmp,1),[size(tmp,1) 1 1])).^2,2)),3),[],1);
        medmat5 = cat(2,medmat5,tmp);
    end
    medmats(ii).dmat = medmat;
    medmats(ii).dmatfilt = medmat2(1:end,1:end,:);
    medmats(ii).Rg = medmat3;
    medmats(ii).Rgfilt = medmat4;
    medmats(ii).dist2cent = medmat5;
end
...
for ii = 1:length(vizidx)
    ...
    imagetriu(medmat(diagidx,diagidx),100,250,flipud(jet),0);
...
%% distance to center of the polymer
figure;
for ii = 1:length(medmats)
    subplot(1,length(medmats),ii);
    hold on;
%     plot(nanmean(medmats(ctrlidx(ii)).dist2cent,2),'.-');
%     plot(nanmean(medmats(ii).dist2cent,2),'.-');
    plot(nanmean(medmats(ii).dist2cent./medmats(ctrlidx(ii)).dist2cent,2),'.-');
    title(dataselected(ii));
%     ylim([3,22]);
end
