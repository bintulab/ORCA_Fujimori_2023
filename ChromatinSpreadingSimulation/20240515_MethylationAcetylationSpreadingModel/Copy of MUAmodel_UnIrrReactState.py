...
def SSA(params,TSTEP,LOOPNUM,DOX,init,loopmat): # , un_init,irr_init,react_init
    # ------------------------------------------------------ #
    ...
    # ---- irreversible or reactivated state transition parameters ---- #  
    irr_thresh = 0.15
    react_thresh = 0.15
    # irr_transition_rate = 0.01
    # react_transition_rate = 0.1
    irr_transition_rate = 0.009
    react_transition_rate = 0.06
    # irr_transition_rate = 0.008
    # react_transition_rate = 0.05
    un_state = xp.ones((NCELL,1),dtype='f')
    irr_state = xp.zeros((NCELL,1),dtype='f')
    react_state = xp.zeros((NCELL,1),dtype='f')
    UIRdynamics = xp.zeros((NCELL,3,LOOPNUM+1),dtype='f')
    UIRdynamics[:,0:1,0] = un_state
    UIRdynamics[:,1:2,0] = irr_state
    UIRdynamics[:,2:3,0] = react_state
    ...
    cumrrates_prv[:,0] = -1;
    cumrrates_end = xp.zeros((NCELL,1),dtype='f')
    avg_A_reporter = xp.zeros((NCELL,1),dtype='f')
    low_A = xp.zeros((NCELL,1),dtype='f')
    high_A = xp.zeros((NCELL,1),dtype='f')
    uir_transition = xp.zeros((NCELL,1),dtype='f')
    ...
    # AwithRW = xp.zeros_like(nucleosome, dtype='f')
    for every in range(1,LOOPNUM+1):
        ...
        while (xp.sum(unfinished,axis=None) != 0):
            ...
            ## calculate avg A to determine irreversible/reactivation threshold
            # avg_A_reporter = xp.mean(A[:,puro_s:Cit_e+1], axis=1).reshape((NCELL,1))
            avg_A_reporter = xp.mean(A[:,pEF_s:Cit_e+1], axis=1).reshape((NCELL,1))
            low_A = (avg_A_reporter < irr_thresh) * un_state
            high_A = (avg_A_reporter >= react_thresh) * un_state

            Aloop = un_state*xp.dot(A,loop) + irr_state*xp.dot(A,loopI) + react_state*xp.dot(A,loopR)
            Mloop = un_state*xp.dot(M,loop) + irr_state*xp.dot(M,loopI) + react_state*xp.dot(M,loopR)
            kbA[:,pEF_s:Cit_e+1] = (1.0-irr_state)*kbA[:,pEF_s:Cit_e+1]
            ...
            cumrrates_end = cumrrates[:,-1:]
            rn0 = xp.tile((cumrrates_end + irr_transition_rate*low_A + react_transition_rate*high_A) * xp.random.random((NCELL,1)),(1,NNUC*2))
            ...
            tau = tau + dtau
            unfinished = xp.sign(xp.sign(TSTEP - tau).astype('f') + 1.0).astype('f')
            nucleosome = nucleosome + xp.tile(unfinished,(1,NNUC))*delta_n
            uir_transition = (xp.sum(delta_n,axis=1) == 0).reshape((NCELL,1))*unfinished
            un_state = un_state - uir_transition
            irr_state = irr_state + uir_transition*low_A
            react_state = react_state + uir_transition*high_A
            cntr = cntr + 1

        wholedynamics[:,:,every] = nucleosome
        UIRdynamics[:,0:1,every] = un_state
        UIRdynamics[:,1:2,every] = irr_state
        UIRdynamics[:,2:3,every] = react_state

    return wholedynamics, UIRdynamics
...
def DefineElementCoordinate(NNUC):
    ...
    puro_s = center-6 + offset; puro_e = center-1 + offset;


def DefineStateSpecificLoop(loop_irr, loop_react):
    # ---- state-specifc loop ---- #
    global loopI,loopR
    loopI = loop_irr
    loopR = loop_react
