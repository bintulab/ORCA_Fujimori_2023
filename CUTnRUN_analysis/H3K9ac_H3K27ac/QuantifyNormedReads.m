%% params
DIR = 'D:\Taihei\CUTnRUN\20230724_KRAB_CNR_H3K9ac_H3K27ac\dedup_scaled_chr19\';
fnames = dir(DIR);
fnames = fnames(3:end);
% file names
...
for ii = 1:length(dnameseries)
    ...
    for jj = 1:length(fnames)
        ...
        if strcmp(dname,dnameseries{ii})
            ...
            title(dname);
            if strcmp(fnamesp{3},'H3K9ac')
                ylim([0,40]);
                ylim([0,6]);
            elseif strcmp(fnamesp{3},'H3K27ac')
                ylim([0,40]);
                ylim([0,6]);
            end
        end
